window.dom = {
    ...
    empty (node) {
        ...
        }*/ while(node.firstChild)array.push(dom.remove(node.firstChild));
        // 返回被删除所有子元素节点作参考验证
        return array;
    },
    /* 改和读-------------------------------- */ // 读写属性,JS允许函数传参数量小于预设,这叫做“重载” overload
    ...
    html (node, string) {
        ...
        else if (argument.lenght === 1) return node.innerHTML;
    },
    // 修改style
    style (node, name, value) {
        ...
        node.style[name] = value;
            ...
            if (typeof name === "string") //dom.style(div, 'color') 获取style的值
    ...
    next (node) {
        ...
        // 存在,略过text文本内容（回车）
        while(curNode && curNode.nodeType === 3)curNode = curNode.nextSibling;
        return curNode;
    ...
    previous (node) {
        ...
        while(curNode && curNode.nodeType === 3)curNode = curNode.previousSibling;
        return curNode;
